...
const Campaign02Table01 = ({ Campaign2items, setCampaign2items, Campaign2itemsStatus }) => {
    return (
        <div>
                    ...
                    {Campaign2items && Campaign2items.map((item) =>
                        item && item.status === 'Pending' && (
                            <tr className="item" key={item.id}>
                                ...
                                <td>{item.Effect}</td>
                                <td>{item.Rank}</td>
                                <td><button className="mark_complete" key={item.id} onClick={() => { Campaign2itemsStatus(item.id, 'Completed'); }}>Move</button></td>
                            </tr>
                        )
                    )}
...
export default Campaign02Table01