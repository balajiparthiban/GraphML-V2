...
export default function TableProvider({ children }) {
    ...
    const updateCustomerTableState = (rowId, status) => {
        const updatedCustomerRecords = customerTableState.reduce((acc, item) => {
            if (item.id === rowId) {
                item.status = status;
            }
            acc.push(item);
        ...
        setCustomerTableState(updatedCustomerRecords);
    };

    return (<>
        <CustomerTableContext.Provider value={{ updateCustomerTableState, customerTableState }}>
            {children}
